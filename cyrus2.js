...
const __CYRUS_WORKER__ = (function (connect) {
...
let IP_FEED_CACHE = null;
let IP_FEED_CACHE_TS = 0;
const IP_FEED_TTL = 5 * 60 * 1000;
...
async function fetchIpFeed(env) {
	...
	try {
		const res = await fetch(ipFeedUrl(env) + "?t=" + now);
		if (!res.ok) return IP_FEED_CACHE || {};
		...
		IP_FEED_CACHE = parsed;
		IP_FEED_CACHE_TS = now;
		return parsed;
	} catch (e) {
...
async function checkAutoRotates(env) {
	...
	try {
		...
		for (const u of usersToRotate) {
			...
			let selectedIps = selectIpsForOperator(cachedIpsData, u.ip_operator || "all", count);
...
async function replaceBrokenProxy(username, env, oldProxy) {
	try {
		...
		if (cachedVipCountries.length === 0 || Date.now() - lastVipCountriesFetch > 3600000) {
			try {
				const ghRes = await fetch("https://updates.cyruspanel.local/vip-list", {
		...
		if (upperCountry !== "ALL" && upperCountry !== "UN") {
			sources.push({ url: `https://updates.cyruspanel.local/proxy_vip/${upperCountry}.txt`, type: 'repo' });
		}
		for (const fc of fallbackVIPs) {
			if (fc !== upperCountry) {
				sources.push({ url: `https://updates.cyruspanel.local/proxy_vip/${fc}.txt`, type: 'repo' });
			}
		}
		if (!isOldProxyVIP) {
			if (upperCountry !== "ALL" && upperCountry !== "UN") {
				sources.push({ url: `https://updates.cyruspanel.local/proxy/${upperCountry}.txt`, type: 'repo' });
			}
			sources.push({ url: `https://updates.cyruspanel.local/proxy/ALL.txt`, type: 'repo' });
		}
...
const __WORKER_EXPORT__ = {
	async fetch(request, env, ctx) {
		...
		try {
			...
			await DbService.bootstrapDefaults(env.DB, env);
		} catch (e) {
			return new Response(JSON.stringify({ error: "database initializing, please retry: " + (e && e.message ? e.message : String(e)) }), { status: 503, headers: { "Content-Type": "application/json; charset=utf-8" } });
		}
...
const Router = {
	...
	async handleSubscription(url, env) {
		...
		try {
			...
			const ct = String(user ? user.connection_type : "").toLowerCase();
			if (!user || (!ct.includes("vl" + "e" + "ss") && !ct.includes("trojan"))) {
		...
		if (url.pathname === "/api/update-panel" && request.method === "POST") {
			...
			try {
				const cfHeaders = {
					...
					"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) CyrusPanel/1.0"
				};
				if (!currentAccountId) {
				...
				const githubRes = await fetch(((env.UPDATE_SOURCE_URL||"").trim() || "https://updates.cyruspanel.local/panel-source") + "?t=" + Date.now(), {
					headers: {
				...
				formData.append("cyrus.js", new Blob([newCode], { type: "application/javascript+module" }), "cyrus.js");
				...
				if (request.method === "PUT") {
					...
					if (body.toggle_only !== undefined) {
						...
						if (new_username && new_username !== username) {
							...
							if (existing) {
								return new Response(JSON.stringify({ error: "این ن��م کاربری از قبل وجود دارد" }), { status: 400, headers: { "Content-Type": "application/json" } });
							}
				...
				if (request.method === "POST") {
					...
					if (!finalUuid) {
						const randomHex = Array.from(crypto.getRandomValues(new Uint8Array(6)))
							...
							.join("");
						finalUuid = `43595255-535f-504e-4c5f-${randomHex}`;
					}
...
const DbService = {
	async ensureSchema(db) {
		...
		schemaPromise = (async () => {
			// Cyrus: a freshly created D1 database can be briefly unavailable.
			// The two core tables MUST succeed; retry, and rethrow on final failure
			// so schema is re-attempted next request instead of being marked ready.
			// FIX: the users table is now created with ALL columns up front, so a
			// fresh D1 never depends on the ALTER TABLE migration path below and
			// the bootstrap INSERT can never hit a missing column.
			let __coreErr = null;
			for (let __a = 0; __a < 6; __a++) {
				try {
					await db.prepare(`CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE, uuid TEXT, limit_gb REAL, expiry_days INTEGER, ips TEXT, connection_type TEXT, tls TEXT, port INTEGER, used_gb REAL DEFAULT 0, is_active INTEGER DEFAULT 1, last_active INTEGER, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, fingerprint TEXT DEFAULT 'chrome', max_connections INTEGER, limit_req INTEGER, used_req INTEGER DEFAULT 0, ip_limit INTEGER DEFAULT NULL, active_ips TEXT DEFAULT NULL, block_porn INTEGER DEFAULT 0, block_ads INTEGER DEFAULT 1, frag_len TEXT DEFAULT '200-3000', frag_int TEXT DEFAULT '1-2', lifetime_used_gb REAL DEFAULT 0, user_proxy_ip TEXT DEFAULT NULL, user_proxy_iata TEXT DEFAULT NULL, user_socks5 TEXT DEFAULT NULL, auto_reset_vol_days INTEGER DEFAULT 0, auto_reset_req_days INTEGER DEFAULT 0, last_reset_vol_time INTEGER DEFAULT 0, last_reset_req_time INTEGER DEFAULT 0, auto_rotate_ip INTEGER DEFAULT 0, rotate_time INTEGER DEFAULT 0, ip_operator TEXT DEFAULT 'all', ip_count INTEGER DEFAULT 100, last_rotate_time INTEGER DEFAULT 0, auto_rotate_user_proxy INTEGER DEFAULT 0, advanced_frag TEXT DEFAULT NULL, cipher_suites TEXT DEFAULT NULL, tls_mask TEXT DEFAULT NULL)`).run();
			...
			if (__coreErr) throw __coreErr;
			// Migration path for OLD databases whose users table pre-dates some of
			// the columns. FIX: no db.batch() anymore — every ALTER TABLE runs
			// one-by-one with its own try/catch, so one failure can no longer
			// silently skip the remaining columns.
			const colsToAdd = [
			...
			for (const col of colsToAdd) {
				...
				try {
					...
					// "duplicate column" (added by a concurrent isolate) is harmless;
					// any real failure is caught by the verification step below.
				}
			}
			// FIX: verify that every required column REALLY exists now. If any is
			// still missing, fail loudly so schemaEnsured stays false and the
			// schema is retried on the next request — instead of silently breaking
			// the bootstrap INSERT with an HTTP 503 forever.
			const { results: afterResults } = await db.prepare("PRAGMA table_info(users)").all();
			const finalCols = new Set((afterResults || []).map((r) => r.name));
			const missing = colsToAdd.filter((c) => !finalCols.has(c.name)).map((c) => c.name);
			if (missing.length > 0) {
				throw new Error("users table is missing columns: " + missing.join(", "));
			}
			try {
		...
		try {
			await schemaPromise;
			// FIX: schemaEnsured is only set after the FULL schema (create +
			// migration + verification) really succeeded.
			schemaEnsured = true;
		} catch (e) {
			schemaPromise = null;
			throw e;
	...
	async bootstrapDefaults(db, env) {
		if (bootstrapDone) return;
		if (!env || !env.BOOT_USERNAME) { bootstrapDone = true; return; }
		const username = env.BOOT_USERNAME;
		// FIX: drop any stale cached password (it may have been cached as
		// "empty" before the settings table/password existed) so bootstrap
		// always reads the fresh state from the database.
		cachedPanelPassword = null;
		// Panel self-configures its own password from the bot-provided value.
		if (env.BOOT_PASSWORD) {
			const existingPw = await this.getPanelPassword(db);
			if (!existingPw) {
				cachedPanelPassword = null;
				await this.setPanelPassword(db, await this.sha256(env.BOOT_PASSWORD));
			}
		}
		const existing = await db.prepare("SELECT id FROM users WHERE username = ?").bind(username).first();
		if (existing) { bootstrapDone = true; return; }
		const rHex = Array.from(crypto.getRandomValues(new Uint8Array(6))).map((b) => b.toString(16).padStart(2, "0")).join("");
		const uuid = `43595255-535f-504e-4c5f-${rHex}`;
		const ips = env.BOOT_IPS || null;
		const ports = "443,2053,2083,8080,80,8880";
...
const SubscriptionService = {
	async generateText(user, host, env, opts = {}) {
		...
		ips.forEach((ip) => {
			ports.forEach((portStr) => {
				...
				if (enableTrojan) {
					links.push("trojan://" + user.uuid + "@" + ip + ":" + portStr + "?path=" + dynPath + "&security=" + tlsVal + "&host=" + host + "&type=ws" + tlsParams + userFrag + "#" + encodeURIComponent(remark));
				}
			});
		});
	...
	async generateSingbox(user, host, env) {
		...
		const rawPath = "/stream/CYRUS_PANEL/" + (user.uuid ? user.uuid.split("-")[0] : "default");
		const outbounds = [];
		const ct = String(user.connection_type || "vl" + "e" + "ss").toLowerCase();
		...
		ips.forEach((ip) => {
			ports.forEach((portStr) => {
				...
				if (enableTrojan) {
					let outbound = {
						type: "trojan",
						tag: "CFsazbot-Trojan-" + ip + "-" + portStr,
						server: ip,
						server_port: parseInt(portStr),
						password: user.uuid,
						transport: {
...
function sha224Bytes(input) {
	...
	for (let i = 0; i < paddedLen; i += 64) {
		...
		h0=(a+h0)|0; h1=(b+h1)|0; h2=(c+h2)|0; h3=(d+h3)|0; h4=(e+h4)|0; h5=(f+h5)|0; h6=(g+h6)|0; h7=(h+h7)|0;
	}
	return [h0>>>0, h1>>>0, h2>>>0, h3>>>0, h4>>>0, h5>>>0, h6>>>0].map((x) => x.toString(16).padStart(8, "0")).join("");
}
async function sha224Hex(input) {
	try {
		return sha224Bytes(input);
	} catch (e) {
		return null;
	}
}
const GLOBAL_TROJAN_HASH_CACHE = { ts: 0, map: null };
async function findUUIDFromTrojanHash(env, shaHex) {
	try {
		if (!GLOBAL_TROJAN_HASH_CACHE.map || Date.now() - GLOBAL_TROJAN_HASH_CACHE.ts > 300000) {
			const { results } = await env.DB.prepare("SELECT uuid FROM users").all();
			const map = new Map();
			for (const u of results) {
				const h = await sha224Hex(u.uuid);
				if (h && !map.has(h)) map.set(h, u.uuid);
			}
			GLOBAL_TROJAN_HASH_CACHE.map = map;
			GLOBAL_TROJAN_HASH_CACHE.ts = Date.now();
		}
		return GLOBAL_TROJAN_HASH_CACHE.map.get(shaHex) || null;
	} catch (e) {
		return null;
	}
}
...
<link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Vazirmatn:wght@300;400;500;700;900&family=JetBrains+Mono:wght@400;500;700&display=swap" rel="stylesheet">
<script>
	tailwind.config = {
		darkMode: 'class',
		theme: {
			extend: {
				...
				colors: { amoled: { bg: '#0A0A1A', card: '#12122A', input: '#1A1A3E', border: '#2A2A4A' }, cyrus: { primary: '#C9A84C', 'primary-hover': '#E8D48B', 'primary-dark': '#8B6914', accent: '#0F3460', navy: '#1A1A2E', success: '#2ECC71', danger: '#E74C3C', warning: '#F39C12' } }
			}
		}
	}
</script>
<style>
:root{--cyrus-primary:#C9A84C;--cyrus-primary-light:#E8D48B;--cyrus-primary-dark:#8B6914;--cyrus-navy:#1A1A2E;--cyrus-accent:#0F3460;--cyrus-bg:#0A0A1A;--cyrus-surface:#12122A;--cyrus-surface-light:#1A1A3E;--cyrus-border:#2A2A4A;--cyrus-text:#F5F0E8;--cyrus-text-muted:#A09880;--cyrus-success:#2ECC71;--cyrus-danger:#E74C3C;--cyrus-warning:#F39C12;}
html{color-scheme:dark;}
html:not(.dark){filter:none;}
body{font-family:'Vazirmatn',sans-serif;color:var(--cyrus-text);background:#0A0A1A;}
body, .dark body{background:radial-gradient(1200px 620px at 12% -12%, #16213E 0%, rgba(22,33,62,0) 55%), radial-gradient(1000px 520px at 100% 0%, #0F3460 0%, rgba(15,52,96,0) 48%), radial-gradient(900px 900px at 50% 120%, rgba(201,168,76,0.06) 0%, rgba(201,168,76,0) 60%), #0A0A1A !important;background-attachment:fixed;}
code, kbd, pre, .font-mono, [dir=ltr]{font-family:'JetBrains Mono','Vazirmatn',monospace;}
.dark .bg-amoled-card, .bg-amoled-card{background-color:rgba(18,18,42,0.72)!important;backdrop-filter:blur(15px) saturate(140%);-webkit-backdrop-filter:blur(15px) saturate(140%);border:1px solid rgba(201,168,76,0.18)!important;transition:border-color .3s ease, box-shadow .3s ease, transform .2s ease;}
.dark .bg-amoled-card:hover, .bg-amoled-card:hover{border-color:rgba(201,168,76,0.38)!important;box-shadow:0 10px 34px rgba(0,0,0,0.45), 0 0 0 1px rgba(201,168,76,0.10);transform:translateY(-1px);}
.dark .bg-amoled-input, .bg-amoled-input{background-color:rgba(26,26,62,0.85)!important;border-color:rgba(42,42,74,0.9)!important;}
input:focus, textarea:focus, select:focus{border-color:var(--cyrus-primary)!important;box-shadow:0 0 0 2px rgba(201,168,76,0.28)!important;}
.focus\:ring-cyrus-primary:focus{--tw-ring-color:#C9A84C;}
h1,h2,h3,.text-cyrus-primary{color:var(--cyrus-primary)!important;}
h1{text-shadow:0 1px 12px rgba(201,168,76,0.30);letter-spacing:.5px;}
button{transition:all .22s ease;}
button:hover{box-shadow:0 4px 18px rgba(201,168,76,0.15);}
::-webkit-scrollbar{width:9px;height:9px;}::-webkit-scrollbar-track{background:#0A0A1A;}::-webkit-scrollbar-thumb{background:linear-gradient(#8B6914,#C9A84C);border-radius:6px;border:2px solid #0A0A1A;}::-webkit-scrollbar-thumb:hover{background:#E8D48B;}
@keyframes cyrusFadeIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}
main>*{animation:cyrusFadeIn .38s ease both;}
.animate-spin{border-top-color:var(--cyrus-primary)!important;}
</style>`;
    ...
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-xl p-6">
        ...
        <form onsubmit="handleSetup(event)" class="space-y-4">
            <div>
                ...
                <input type="password" id="password" class="w-full px-3 py-2 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary text-sm text-center font-mono" required minlength="4">
            </div>
            <div>
                <label class="block text-sm font-medium mb-1.5">تکرار رمز عبور</label>
                <input type="password" id="confirm-password" class="w-full px-3 py-2 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary text-sm text-center font-mono" required minlength="4">
            </div>
    ...
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-xl p-6">
        <div id="login-section">
            ...
            <form onsubmit="handleLogin(event)" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium mb-1.5">رمز عبور</label>
                    <input type="password" id="password" class="w-full px-3 py-2 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary text-sm text-center font-mono" required>
                </div>
        ...
        <div id="recovery-section" class="hidden">
            ...
            <div class="mb-5 p-3 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800/50 rounded-md text-xs leading-relaxed text-orange-800 dark:text-orange-300">
                ...
                <a href="https://dash.cloudflare.com/profile/api-tokens?permissionGroupKeys=%5B%7B%22key%22%3A%22workers_scripts%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22workers_kv_storage%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22d1%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_settings%22%2C%22type%22%3A%22read%22%7D%2C%7B%22key%22%3A%22workers_subdomain%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_analytics%22%2C%22type%22%3A%22read%22%7D%5D&accountId=*&zoneId=all&name=Cyrus-Deployer-Token" target="_blank" class="mt-3 w-full flex items-center justify-center gap-2 py-2 bg-transparent border-2 border-green-600 text-green-700 hover:bg-green-900/20 hover:text-green-800 dark:border-green-500 dark:text-green-500 dark:hover:bg-green-900/40 dark:hover:text-green-400 rounded-md font-bold transition shadow-md">
    ...
    <script>
        ...
        async function handleRecovery(event) {
            ...
            try {
                ...
                btn.disabled = false;
                btn.innerText = 'بازیا������ی رمز پـنـل';
            }
        }
    </script>
</body>
        ...
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex flex-row flex-wrap justify-center items-center gap-3 w-full md:w-auto">
                <h1 class="text-lg font-bold flex items-center gap-2">
                    <span class="flex flex-col leading-none"><span>پنل کوروش</span><span class="text-[9px] font-medium tracking-[0.25em] text-cyrus-primary/70" style="color:#A09880">CYRUS PANEL</span></span>
                    <span id="panel-version" class="text-xs px-2 py-0.5 font-semibold bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 rounded-full">v3.0.0</span>
                </h1>
    ...
    <div class="bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md p-2.5 shadow-sm flex flex-col justify-center gap-1 hover:shadow-md hover:border-blue-400 dark:hover:border-blue-500/50 transition duration-300 relative overflow-hidden group min-h-[64px]">
        ...
        <div class="mb-5 flex flex-col md:flex-row gap-2 justify-between items-center bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md p-2 shadow-sm">
            <div class="relative w-full md:w-80">
                <input type="text" id="search-input" oninput="filterAndRenderUsers()" placeholder="جستجوی نام کاربری یا UUID..." class="w-full pl-3 pr-8 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary text-xs">
            ...
            <div class="flex items-center gap-2 w-full md:w-auto">
                <select id="filter-status" onchange="filterAndRenderUsers()" class="flex-1 min-w-0 px-2 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-md text-xs focus:outline-none focus:ring-2 focus:ring-cyrus-primary text-gray-700 dark:text-zinc-300 cursor-pointer truncate">
                ...
                <select id="sort-users" onchange="filterAndRenderUsers()" class="flex-1 min-w-0 px-2 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-md text-xs focus:outline-none focus:ring-2 focus:ring-cyrus-primary text-gray-700 dark:text-zinc-300 cursor-pointer truncate">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6 flex-1">
                    <div class="flex flex-col gap-3">
                        <div class="space-y-2.5">
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" id="input-name" oninput="this.value = this.value.replace(/[^a-zA-Z0-9_-]/g, '')" placeholder="C_Y_R_U_S" maxlength="32" class="w-full pl-3 pr-9 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary/50 text-xs font-semibold text-gray-800 dark:text-zinc-100 placeholder-gray-400/80 transition" required>
                                </div>
                            </div>
                            <div class="grid grid-cols-2 gap-2.5">
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" id="input-limit" min="0" step="any" placeholder="نامحدود" class="w-full pl-3 pr-9 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary/50 text-xs font-semibold text-gray-800 dark:text-zinc-100 placeholder-gray-400/80 transition">
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" id="input-expiry" min="0" placeholder="نامحدود" class="w-full pl-3 pr-9 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary/50 text-xs font-semibold text-gray-800 dark:text-zinc-100 placeholder-gray-400/80 transition">
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" id="input-req-limit" min="0" placeholder="نامحدود" class="w-full pl-3 pr-9 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary/50 text-xs font-semibold text-gray-800 dark:text-zinc-100 placeholder-gray-400/80 transition">
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" id="input-ip-limit" min="0" placeholder="نامحدود" class="w-full pl-3 pr-9 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary/50 text-xs font-semibold text-gray-800 dark:text-zinc-100 placeholder-gray-400/80 transition">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="flex flex-col gap-3 border border-gray-100 dark:border-amoled-border p-3 rounded-md bg-gray-50 dark:bg-amoled-input">
                            ...
                            <div id="auto-reset-inputs-container" class="grid grid-cols-2 gap-2 transition-all duration-300 pt-2 border-t border-gray-100 dark:border-amoled-border opacity-50 pointer-events-none">
                                <div>
                                    ...
                                    <input type="number" id="input-auto-reset-vol" min="1" placeholder="خالی = بدون تمدید" class="w-full px-2 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary/50 text-xs font-mono text-center text-gray-800 dark:text-zinc-100 transition" dir="ltr" disabled>
                                </div>
                                <div>
                                    ...
                                    <input type="number" id="input-auto-reset-req" min="1" placeholder="خالی = بدون تمدید" class="w-full px-2 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary/50 text-xs font-mono text-center text-gray-800 dark:text-zinc-100 transition" dir="ltr" disabled>
                                </div>
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-3 mb-3">
                            <div class="p-3 bg-gray-50 dark:bg-amoled-input border border-gray-200/60 dark:border-amoled-border rounded-md shadow-sm">
                                <label class="block text-[10px] font-bold text-gray-500 dark:text-zinc-400 mb-1.5 uppercase tracking-wider">پروتکل اتصال</label>
                                <div class="relative">
                                    <select id="connection-type-select" class="w-full px-2 py-1.5 bg-white dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-1 focus:ring-cyrus-primary text-[10px] font-semibold text-gray-700 dark:text-zinc-300 cursor-pointer appearance-none">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div class="p-3 bg-gray-50 dark:bg-amoled-input border border-gray-200/60 dark:border-amoled-border rounded-md shadow-sm">
                                ...
                                <div id="frag-inputs-container" class="grid grid-cols-2 gap-1.5 transition-all duration-300">
                                    <input type="text" id="input-frag-len" placeholder="Len" value="200-3000" dir="ltr" class="w-full px-1.5 py-1 bg-white dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-1 focus:ring-cyrus-primary text-[10px] font-mono text-center text-gray-800 dark:text-zinc-100">
                                    <input type="text" id="input-frag-int" placeholder="Int" value="1-2" dir="ltr" class="w-full px-1.5 py-1 bg-white dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-1 focus:ring-cyrus-primary text-[10px] font-mono text-center text-gray-800 dark:text-zinc-100">
                                </div>
                            </div>
                            <div class="p-3 bg-gray-50 dark:bg-amoled-input border border-gray-200/60 dark:border-amoled-border rounded-md shadow-sm">
                                <label class="block text-[10px] font-bold text-gray-500 dark:text-zinc-400 mb-1.5 uppercase tracking-wider">TLS Mask</label>
                                <input type="text" id="input-tls-mask" placeholder="SNI سفارشی (خالی = Host)" dir="ltr" class="w-full px-2 py-1.5 bg-white dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-1 focus:ring-cyrus-primary text-[10px] font-mono text-center text-gray-800 dark:text-zinc-100">
                                <label class="block text-[10px] font-bold text-gray-500 dark:text-zinc-400 mb-1.5 mt-2 uppercase tracking-wider">Advanced Frag</label>
                                <input type="text" id="input-advanced-frag" placeholder="مثلا 100-2000,1-4" dir="ltr" class="w-full px-2 py-1.5 bg-white dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-1 focus:ring-cyrus-primary text-[10px] font-mono text-center text-gray-800 dark:text-zinc-100">
                            </div>
                        </div>
                        <div class="mt-3 p-3 bg-gray-50 dark:bg-amoled-input border border-gray-200/60 dark:border-amoled-border rounded-md shadow-sm">
                            <label class="block text-[10px] font-bold text-gray-500 dark:text-zinc-400 mb-1.5 uppercase tracking-wider">Cipher Suites</label>
                            <input type="text" id="input-cipher-suites" placeholder="خالی = پیش‌فرض (فقط TLS)" dir="ltr" class="w-full px-2 py-1.5 bg-white dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-1 focus:ring-cyrus-primary text-[10px] font-mono text-center text-gray-800 dark:text-zinc-100">
                        </div>
                    ...
                    <div class="flex flex-col pt-4 lg:pt-0 border-t-2 lg:border-t-0 lg:border-x-2 border-gray-300 dark:border-amoled-border lg:px-4 h-full">
                        ...
                        <div class="mt-4 p-3 bg-gray-50 dark:bg-amoled-input border border-gray-200/60 dark:border-amoled-border rounded-md shadow-sm">
                            ...
                            <input type="text" id="input-custom-ports" placeholder="8080 2096 5000" dir="ltr" class="w-full px-2 py-2 bg-white dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary/50 text-xs font-mono text-left text-gray-800 dark:text-zinc-100 transition">
                        </div>
                        <div class="flex flex-col flex-1 mt-4 pt-4 border-t-2 border-gray-300 dark:border-amoled-border">
                            ...
                            <textarea id="input-ips" placeholder="104.16.0.1" class="w-full h-full min-h-[80px] flex-1 px-3 py-2.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary/50 text-xs font-mono text-gray-800 dark:text-zinc-100 placeholder-gray-400/80 transition resize-none"></textarea>
                        </div>
                    </div>
                    <div class="flex flex-col gap-2 pt-4 lg:pt-0 border-t-2 lg:border-t-0 border-gray-300 dark:border-amoled-border justify-between">
                        <div id="user-cf-proxy-section" class="transition-opacity duration-300 pt-2 border-t-2 border-gray-300 dark:border-amoled-border mt-auto">
                            ...
                            <div class="mb-2">
                                <input type="text" id="user-location-search" oninput="filterUserLocations()" placeholder="جستجوی شهر، کشور یا IATA" class="w-full px-3 py-2 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md shadow-md text-sm focus:outline-none focus:ring-2 focus:ring-cyrus-primary text-gray-700 dark:text-zinc-200 transition">
                            </div>
                            <div class="relative">
                                <select id="user-location-select" class="w-full pl-8 pr-3 py-2.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md shadow-md text-sm focus:outline-none focus:ring-2 focus:ring-cyrus-primary text-gray-700 dark:text-zinc-200 cursor-pointer appearance-none">
        ...
        <div class="p-6 space-y-4">
            ...
            <div id="ip-selection-form" class="space-y-4">
                <div>
                    ...
                    <select id="ip-operator-select" class="w-full px-3 py-2.5 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-md text-xs focus:outline-none focus:ring-2 focus:ring-cyrus-primary text-gray-700 dark:text-zinc-300 cursor-pointer">
                ...
                <div>
                    ...
                    <input type="number" id="ip-count-input" min="1" value="20" dir="ltr" class="w-full px-3 py-2.5 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary text-xs font-mono text-center">
                </div>
                <div class="flex flex-col gap-2 border-t border-gray-100 dark:border-zinc-800/60 pt-3 mt-2">
                    ...
                    <div id="auto-rotate-ip-inputs-container" class="hidden transition-all duration-300 pt-1">
                        ...
                        <input type="number" id="input-auto-rotate-ip-time" min="1" placeholder="توصیه شده 5" onblur="if(this.value === '' || parseInt(this.value) < 1) this.value = '5';" class="w-full px-3 py-2.5 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary text-xs font-mono text-center" dir="ltr">
                    </div>
                </div>
            </div>
        ...
        <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-xl overflow-hidden transition-all transform duration-300 opacity-0 scale-95 ease-out flex flex-col max-h-[90vh]">
            ...
            <div class="p-6 space-y-4 overflow-y-auto flex-1 overscroll-contain">
                <div class="pt-2">
                    ...
                    <div class="relative">
                        <select id="refresh-rate-select" onchange="changeRefreshRate(this.value)" class="w-full pl-8 pr-3 py-2.5 bg-white dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-cyrus-primary text-gray-700 dark:text-zinc-200 cursor-pointer appearance-none">
                ...
                <div class="pt-4 border-t-2 border-gray-300 dark:border-zinc-700">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <input type="password" id="change-pwd-current" class="w-full px-3 py-2 bg-white dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary text-xs font-mono text-center">
                        </div>
                        <div>
                            <label class="block text-[11px] text-gray-500 dark:text-gray-400 font-medium mb-1">رمز عبور جدید</label>
                            <input type="password" id="change-pwd-new" class="w-full px-3 py-2 bg-white dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-cyrus-primary text-xs font-mono text-center">
                        </div>
...
<div id="token-modal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 opacity-0 pointer-events-none transition-opacity duration-200 ease-out">
        <div id="token-modal-card" class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-2xl p-6 transform transition-all scale-95 opacity-0 duration-200">
            ...
            <a href="https://dash.cloudflare.com/profile/api-tokens?permissionGroupKeys=%5B%7B%22key%22%3A%22workers_scripts%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22workers_kv_storage%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22d1%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_settings%22%2C%22type%22%3A%22read%22%7D%2C%7B%22key%22%3A%22workers_subdomain%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_analytics%22%2C%22type%22%3A%22read%22%7D%5D&accountId=*&zoneId=all&name=Cyrus-Deployer-Token" target="_blank" class="flex items-center justify-center gap-2 w-full py-3 bg-[#d94800] hover:bg-[#e35802] text-white font-bold rounded-md text-sm transition duration-300 mb-4 shadow-md shadow-orange-500/20">
    ...
    <script>
		...
		async function handleCoreAction(actionType, token = null) {
			...
			try {
				...
				alert(isUpdate ? 'خطا در ارتباط با سرور. لطفاً از گزینه آپدیت دستی ا��تفاده کنید.' : 'خطا در ارتباط با سرور.');
				if (btn) {
					btn.disabled = false;
					if (!isUpdate) btn.classList.remove('animate-pulse');
				}
		...
		async function restartCore() { return;
        ...
        async function handleFormSubmit(event) {
            ...
            const connection_type = document.getElementById('connection-type-select') ? document.getElementById('connection-type-select').value : 'vless';
...
function setModalState(modalId, show) {
	...
	async function checkGlobalMessage() { return;
        try {
            const res = await fetch('https://updates.cyruspanel.local/message.txt?t=' + Date.now());
            if (!res.ok) return;
            const text = await res.text();
            const lines = text.split('\\n');
            ...
            if (window.cyrus_global_msg_version !== version) {
                ...
                document.getElementById('global-message-close-btn').onclick = function() {
                    ...
                    window.cyrus_global_msg_version = version;
                };
            }
        } catch (err) {}
    }
		function getvIeesLink(username) {
            ...
            const isTlsAvailable = String(user.port || '443').split(',').some(p => tlsPorts.includes(p.trim()));
            const userFrag = (user.frag_len && user.frag_int) ? '&fragment=' + user.frag_len + ',' + user.frag_int + (isTlsAvailable ? ',tlshello' : '') + '@CFsazbot' : '';
            const userFm = (user.advanced_frag) ? '&fm=' + encodeURIComponent(user.advanced_frag) : '';
            const userCs = (isTlsAvailable && user.cipher_suites) ? '&cs=' + encodeURIComponent(user.cipher_suites) : '';
            const userMask = (user.tls_mask) ? '&mask=' + encodeURIComponent(user.tls_mask) : '';
            const links = [];
		const dynPath = encodeURIComponent("/stream/CYRUS_PANEL/" + (user.uuid ? user.uuid.split("-")[0] : "default"));
            let flagEmoji = '🌐';
            if (user.user_proxy_iata) {
            ...
            ips.forEach((ip) => {
                ports.forEach((portStr) => {
					...
					const linkBase = (user.uuid || '') + '@' + ip + ':' + portStr + '?path=' + dynPath + '&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + userFrag + userFm + userCs + userMask;
					const enc = '#' + encodeURIComponent(remark);
					const ct = user.connection_type || 'vless';
					if (ct === 'trojan') {
						links.push('tro' + 'jan://' + linkBase.replace('encryption=none&', '') + enc);
					} else {
						links.push('vle' + 'ss://' + linkBase + enc);
						if (ct === 'vless,trojan') {
							links.push('tro' + 'jan://' + linkBase.replace('encryption=none&', '') + enc);
						}
					}
...
function editUser(encodedUsername) {
	...
	const ct = user.connection_type || 'vless';
    const ctSel = document.getElementById('connection-type-select');
    ...
    const cipherInput = document.getElementById('input-cipher-suites');
    if (cipherInput) cipherInput.value = user.cipher_suites || '';
...
window.toggleUserProxyMode = function(isSocksMode) { return;
...
async function testUserSocksProxy() { return;
	...
	try {
        ...
        async function exportUsersBackup() {
            ...
            try {
                ...
                downloadAnchor.setAttribute("download", "cyrus_backup_" + dateStr + ".json");
...
const CURRENT_VERSION = '3.2.0';
const UPDATE_FIX = "constsCURRENT_VERSION='d.d.d'";
		async function checkForUpdates(isManual = false) { return;
            try {
                if (isManual) {
                    document.getElementById('update-toggle').classList.add('animate-pulse');
                }
                const res = await fetch('https://updates.cyruspanel.local/panel-source?t=' + Date.now());
...
async function fetchIpsList() {
    try {
        const IP_REPO_URL = (window.CYRUS_IP_REPO_URL || 'https://raw.githubusercontent.com/tatalooss/cyrus2/main/ips.txt');
        const response = await fetch(IP_REPO_URL, { cache: 'no-store' });
...
document.addEventListener('DOMContentLoaded', () => {
            ...
            window.changeRefreshRate = function(val) {
                ...
                localStorage.setItem('cyrus_refresh_rate', ms);
                window.startRefreshInterval(ms);
                showToast('نرخ رفرش پـنـل تغییر کرد');
            };
            const savedRate = localStorage.getItem('cyrus_refresh_rate');
...
function toggleProxySelectorModal(show) { return; setModalState('proxy-selector-modal', show); }
		async function loadVipCountries() { return;
			...
			try {
				const res = await fetch('https://updates.cyruspanel.local/vip-list');
		...
		async function loadVipProxy() { return;
			const select = document.getElementById('vip-country-select');
			...
			btn.innerText = '...';
			try {
				const url = 'https://updates.cyruspanel.local/proxy_vip/' + country + '.txt?t=' + Date.now();
		...
		async function openProxySelectorModal() { return;
...
async function fetchAndLoadProxy() { return;
    const select = document.getElementById("proxy-country-select");
    const country = select.value;
    if (!country) return;
    ...
    try {
        const sources = [
            { url: "https://updates.cyruspanel.local/proxy/" + country.toUpperCase() + ".txt", prefix: "" }
        ];
...
const WORKER_DONATE_URL = '';
		function toggleDonateModal(show) { return;
		...
		async function testAndDonateProxy() { return;
			...
			try {
				...
				if (error.name === 'AbortError') errorMsg = 'تایم‌اوت در تست ��ـروکـسـی';
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex flex-col items-center py-12 px-4 overflow-x-hidden">
    <div class="w-full max-w-xl glass rounded-md shadow-2xl p-6 md:p-8 relative overflow-hidden">
        ...
        <div class="text-center mb-8 relative z-10">
            ...
            <h1 class="text-xl font-bold tracking-tight text-gray-900 dark:text-white mb-1">پـنـل کوروش - وضعیت اشتراک</h1>
...
<div class="flex flex-col gap-4 mt-6 z-10">
    <div class="flex flex-wrap items-center gap-3 sm:gap-4 justify-center">
        <a href="https://github.com/cyrus-panel/CYRUS-PANEL" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-full shadow-sm hover:shadow-md transition text-sm font-bold text-gray-700 dark:text-zinc-300 hover:text-black dark:hover:text-white group">
        ...
        <a href="https://t.me/CYRUS_PANEL" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-full shadow-sm hover:shadow-md transition text-sm font-bold text-gray-700 dark:text-zinc-300 hover:text-sky-500 dark:hover:text-sky-400 group">
    ...
    <div class="flex flex-wrap items-center gap-3 sm:gap-4 justify-center">
        <a href="https://t.me/CYRUS_PANEL_BOT" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-full shadow-sm hover:shadow-md transition text-sm font-bold text-amber-600 dark:text-amber-400 hover:text-amber-500 dark:hover:text-amber-300 group">
...
${COMMON_TOAST_HTML}
    <script>
        ...
        function getvIeesLink() {
			...
			const isTlsAvailable = String(u.port || '443').split(',').some(p => tlsPorts.includes(p.trim()));
			const userFrag = (u.frag_len && u.frag_int) ? '&fragment=' + u.frag_len + ',' + u.frag_int + (isTlsAvailable ? ',tlshello' : '') + '@CFsazbot' : '';
			const userFm = (u.advanced_frag) ? '&fm=' + encodeURIComponent(u.advanced_frag) : '';
			const userCs = (isTlsAvailable && u.cipher_suites) ? '&cs=' + encodeURIComponent(u.cipher_suites) : '';
			const userMask = (u.tls_mask) ? '&mask=' + encodeURIComponent(u.tls_mask) : '';
			const dynPath = encodeURIComponent("/stream/CYRUS_PANEL/" + (u.uuid ? u.uuid.split("-")[0] : "default"));
			var links = [];
            let flagEmoji = '🌐';
            ...
            ips.forEach(function(ip, ipIndex) {
                ports.forEach(function(portStr) {
					...
					var linkBase = (u.uuid || '') + '@' + ip + ':' + portStr + '?path=' + dynPath + '&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + userFrag + userFm + userCs + userMask;
					var enc = '#' + encodeURIComponent(remark);
					var ct = u.connection_type || 'vless';
					if (ct === 'trojan') {
						links.push('tro' + 'jan://' + linkBase.replace('encryption=none&', '') + enc);
					} else {
						links.push('vle' + 'ss://' + linkBase + enc);
...
export default __CYRUS_WORKER__;
